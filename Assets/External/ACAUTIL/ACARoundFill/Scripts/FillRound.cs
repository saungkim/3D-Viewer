...
public class FillRound : MonoBehaviour
{
    ...
    void RefreshAngle()
    {
        ...
        this.ImgStartDot.transform.localRotation = Quaternion.identity;
        this.ImgStartDot.transform.Translate(0, this.distance, 0);

        this.ImgEndDot.transform.localPosition = Vector3.zero;
        this.ImgEndDot.transform.localRotation = Quaternion.identity;
        //this.ImgEndDot.transform.Rotate(0, 0, -this.GetAngle(ratio), Space.Self);
        Vector3 endDotEuler = this.ImgEndDot.transform.localEulerAngles;
        endDotEuler.z = -this.GetAngle(ratio);

        this.ImgEndDot.transform.localEulerAngles = endDotEuler;
